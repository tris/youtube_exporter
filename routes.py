...
from config import VIDEO_ID_PATTERN, CHANNEL_ID_PATTERN, YOUTUBE_API_KEY, DEFAULT_INTERVAL, MIN_INTERVAL
from metrics import update_metrics, update_channel_metrics, get_prometheus_metrics
...
def metrics():
    ...
    if not video_id and not channel_id:
        return Response("Missing required query parameter: video_id (YouTube video ID) or channel (YouTube channel ID)", status=400)
    if video_id and channel_id:
        return Response("Provide either video_id (YouTube video ID) or channel (YouTube channel ID), not both", status=400)
