...
def fetch_two_spaced_frames(url, frame_skip=DEFAULT_FRAME_SKIP, max_height=None):
    ...
    try:
        ...
        logger.warning(f"Invalid max_height value: {max_height}, using default")
        max_height = MAX_VIDEO_HEIGHT
    ...
    try:
        ...
        logger.warning(f"Failed to create format string with max_height {max_height}: {e}")
        ydl_opts = {
            'format': 'best',
            'quiet': True,
            'no_warnings': True,
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if 'formats' in info and info['formats']:
                ...
                formats = [f for f in info['formats'] if f.get('height', 0) > 0]
                if not formats:
                ...
                logger.debug(f"Stream URL: {stream_url[:100]}...")  # Log first 100 chars of URL
                ...
                is_manifest = False
                try:
                    with urllib.request.urlopen(stream_url, timeout=5) as check_response:
                        first_bytes = check_response.read(100)
                        if b'#EXTM3U' in first_bytes or b'<MPD' in first_bytes:
                            is_manifest = True
                            logger.info("Stream URL points to HLS/DASH manifest file")
                        elif first_bytes.decode('utf-8', errors='ignore').strip().startswith('#'):
                            is_manifest = True
                            logger.info("Stream URL appears to be a playlist/manifest")
                except Exception as e:
                    logger.debug(f"Failed to check stream URL: {e}")

                if is_manifest:
                    # Handle manifest-based streams - measure actual bitrate from segments
                    logger.info("Processing HLS/DASH manifest for measured bitrate")
                    try:
                        import m3u8
                        playlist = m3u8.load(stream_url)
                        if playlist.segments:
                            logger.debug(f"Found {len(playlist.segments)} segments in HLS playlist")
                            ...
                            for segment in playlist.segments:
                                if total_segment_duration >= target_duration:
                                    break
                                try:
                                    with urllib.request.urlopen(segment.uri, timeout=5) as seg_response:
                                        ...
                                        segment_duration = getattr(segment, 'duration', 6.0)
                                        total_segment_duration += segment_duration
                                        logger.debug(f"Downloaded video segment {segments_downloaded}: {len(seg_data)} bytes ({segment_duration}s duration)")
                                except Exception as e:
                                    logger.debug(f"Failed to download segment: {e}")
                                    continue

                            if segment_bytes > 0 and total_segment_duration > 0:
                                # Calculate measured bitrate from actual video segments
                                bitrate = (segment_bytes * 8) / total_segment_duration
                                logger.info(f"Measured bitrate: {bitrate:.0f} bps ({segment_bytes} bytes for {total_segment_duration:.2f}s of content)")
                            else:
                                logger.error("Could not download video segments for bitrate measurement")
                                bitrate = None
                        else:
                    ...
                    if cap.isOpened():
                        ...
                        logger.error("Could not open manifest stream for frame capture")
                        frame1 = frame2 = None
                else:
                    ...
                    try:
                        ...
                        logger.debug(f"Starting download measurement for {playback_duration:.2f}s using urllib")
                        try:
                            with urllib.request.urlopen(stream_url, timeout=10) as response:
                                ...
                                while time.time() - download_start < playback_duration:
                                    ...
                                    if not chunk:
                                        logger.debug(f"Download ended after {chunks_read} chunks (empty chunk)")
                                        break
                        ...
                        if bytes_downloaded > 0:
                            bitrate = (bytes_downloaded * 8) / playback_duration
                            logger.info(f"Measured bitrate: {bitrate:.0f} bps ({bytes_downloaded} bytes for {playback_duration:.2f}s of content)")
                        else:
                            logger.error("Could not download data for bitrate measurement")
                            bitrate = None
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if 'formats' in info and info['formats']:
                    ...
                    logger.info(f"Selected format for object detection: {resolution} (codec: {selected_codec})")
                    ...
                    cap = cv2.VideoCapture(stream_url)
                    if not cap.isOpened():
                        logger.error(f"Failed to open stream URL: {stream_url} (codec: {selected_codec})")
                        logger.warning("This might be due to an incompatible codec. Trying alternative formats...")

                        # Try alternative formats if the selected one fails
                        for alt_fmt in sorted(info['formats'], key=lambda f: f.get('height', 0) or 0, reverse=True)[1:4]:  # Next 3 best
                            alt_height = alt_fmt.get('height', 0)
                            alt_codec = alt_fmt.get('vcodec', 'unknown')
                            if alt_height > 0 and alt_height != format.get('height', 0):
                                logger.info(f"Trying alternative format: {alt_fmt.get('width', 0)}x{alt_height} (codec: {alt_codec})")
                                try:
                                    ...
                                    if alt_cap.isOpened():
                                        ...
                                        alt_cap.release()
                                        if alt_ret:
                                            logger.info(f"Successfully captured frame with alternative format: {alt_fmt.get('width', 0)}x{alt_height}")
                                            frame = alt_frame
                                            resolution = f"{alt_fmt.get('width', 0)}x{alt_height}"
                                            ret = True
                                            break
                                except Exception as e:
                                    logger.debug(f"Alternative format failed: {e}")
                                    continue
            ...
            logger.error(f"Unexpected error fetching frame for object detection: {e}")
            return None
